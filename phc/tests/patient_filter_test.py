...
from phc.web.patient_filter_query_builder import PatientFilterQueryBuilder, ObservationProperty, Property
...
class PatientFilterQueryBuilderTest(unittest.TestCase):
    ...
    def test_patient_filter_observation_component(self):
        ...
        filter_obj \
            .patient() \
            .with_observations([
                ObservationProperty().value_codeable_concept_code(eq=['LA10316-0', 'LA10315-2']),
                ObservationProperty().with_components(
                    [
                        Property().code(eq='allele1').system(eq='http://regenstrief.org/pgx').value_string(eq='C'),
                        Property().code(eq='allele2').system(eq='http://regenstrief.org/pgx').value_string(eq='G')
                    ]
                )
            ])

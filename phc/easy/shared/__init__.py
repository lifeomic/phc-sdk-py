...
from phc.easy.patients.codeable import expand_codeable_column

CODE_COLUMNS = ['meta', 'identifier', 'extension', 'telecom']
DATE_COLUMNS = ['dob', 'birth_date', 'birthDate', 'deceasedDateTime']


def __column_to_frame(frame: pd.DataFrame, column_name: str, expand_func):
...
def expand_data_frame(
    ...
    code_frames = [expand_codeable_column(frame[col_name])
                   for col_name in codeable_col_names]

    custom_names = [key for key, _func in custom_columns if key in frame.columns]

    combined = pd.concat([
        *[__column_to_frame(frame, key, func) for key, func in custom_columns],
